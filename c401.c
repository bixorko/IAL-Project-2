...
**/
    ...
    if ((*RootPtr) == NULL){
        ...
        *RootPtr = NULL;
        ...
        tBSTNodePtr min = *RootPtr;
        ReplaceByRightmost(min, &(*RootPtr)->LPtr);
        (*RootPtr)->Key = min->Key;
        (*RootPtr)->BSTNodeCont = min->BSTNodeCont;
        BSTDelete(&(*RootPtr)->LPtr, min->Key);
        return;
    }
}
